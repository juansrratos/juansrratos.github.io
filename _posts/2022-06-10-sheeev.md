...
(1) A **presheaf** $$\mathscr{F}$$ of sets on on $$X$$ consists of producing a set $$\mathscr{F} (U)$$ for every open subset $$U$$ of $$X$$, and a morphism $$\rho_{U, V} \colon \mathscr{F} (U) \to \mathscr{F} (V)$$ for every pair of open subsets $$V \subseteq U$$. Furthermore, $$\mathscr{F}$$ satisfies the following properties:
1. $$\mathscr{F} (\emptyset) = 0$$,   
2. $$\rho_{U, U} = \text{id}_{\mathscr{F}(U)}$$ for all open subsets $$U \subset X$$, and   
